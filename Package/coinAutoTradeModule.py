...
class CoinAutoTradeModule:
    ...
    def get_ma5_checked_try_symbol_list(self, try_symbol_list):
        """구매희망종목 중 5일 이평선이상종목"""
    ...
    def get_today_plan_to_buy_list(self, ma5_checked_try_symbol_list_list):
    ...
    def get_coin_balance_list(self):
        """매수종목리스트"""
        bought_list = [x['currency'] for x in upbit.get_balances() if x['currency'] != "KRW"]
        return bought_list

    def check_target_alert(self, try_symbol_list):
        """접속확인 및 타겟프라이스 확인"""
        ma5_checked_try_symbol_list = []
        ma5_checked_try_symbol_list = self.get_ma5_checked_try_symbol_list(try_symbol_list)
    ...
    def send_buy_order(self, today_plan_to_buy_coin, today_plan_to_buy_list_list, target_price, fluid_buy_amount):
        """매수"""
        print(f"구매직전구매목록: {today_plan_to_buy_list_list}/구매직전타겟가: {target_price}/구매직전요청금액: {fluid_buy_amount}")
        self.send_message(f"구매직전구매목록: {today_plan_to_buy_list_list}/구매직전타겟가: {target_price}/구매직전요청금액: {fluid_buy_amount}")
        ...
        self.check_target_alert(today_plan_to_buy_list_list)
    ...
    def send_all_balances_sell_order(self, bought_list):
        """전량매도"""
        for sym in bought_list:
            coin_balance = self.get_balances(sym)
    ...
    def terget_time_buy_coin_sell(self, total_value_rate):
    ...
    #                 send_buy_order(today_plan_to_buy_coin, today_plan_to_buy_list_list, target_price, fluid_buy_amount)
    #         else:
    #             send_all_balances_sell_order(bought_list)
    #         time.sleep(1)
